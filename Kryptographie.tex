...
Dann vergleichen $|C| \geq k$? Ausgabe JA, falls $|C| \geq k$ sonst NEIN
...
\item [Aufgabe:] Bestimme die maximalzahl $\omega (G)$\footnote{$\omega(G) = max \{|C|: C ist eine Clique in G\}$ ist die Cliquenzahl} von paarweise verbundenen Knoten in G
\end{description}
\end{framed}
\end{description}
\textbf{Behauptung:} CLIQUE in polynomieller Zeit lösbar $\Rightarrow^{(1)}$ CLIQUE-Zwischenversion in polizeit lösbar $\Rightarrow^{(2)}$ CLIQUE-Suchversion in polynomieller Zeit lösbar.
